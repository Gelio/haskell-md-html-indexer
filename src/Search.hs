...
search (SearchOptions phrase cmd) input = do
  matches <- runConduitRes $ readIndexFile input .| filterIndex phrase .| sinkList
  if null cmd then
    displayMatches matches
  else
    execMatches cmd matches

readIndexFile :: FilePath -> ConduitT i IndexMatch (ResourceT IO) ()
readIndexFile path = sourceFile path
  .| decodeUtf8C
  .| linesUnboundedC
  .| mapC (T.break (=='\ETB'))
...
displayMatches matches = putStrLn ("Found " ++ show (length matches) ++ " matches:") >> mapM_ displayMatch matches
